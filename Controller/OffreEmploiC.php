<?php
    ...
    class OffreEmploiC{
        ...
        public function listOffresOnce(){
            try {
                $pdo = config::getConnexion();
                $query = "SELECT * FROM offre_demploi JOIN candidats ON offre_demploi.cin = candidats.cin";
                $statement = $pdo->query($query);
                $offreEmplois = $statement->fetchAll(PDO::FETCH_ASSOC);
                return $offreEmplois;
            } catch(PDOException $e) {
        ...
        public function updateCin($offreEmploi,$cin){
            try{
            $pdo = config::getConnexion();
            $sql = 'UPDATE `offre_demploi` SET `cin_c`=:cin_c WHERE cin_c=:old_cin';
            $list = $pdo->prepare($sql);
            $old_cin= $offreEmploi->getCin_c();
            $list->execute([
                'old_cin' => $old_cin,
                'cin_c' => $cin,
            ]);
